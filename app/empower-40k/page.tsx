...
export default function EmpowerPage() {
  return (
    <main className="min-h-screen bg-white text-navy">
      <section className="border-b border-navy/10 bg-[linear-gradient(135deg,_rgba(20,90,59,0.1),_rgba(255,255,255,1))]">
        <div className="mx-auto max-w-6xl px-6 py-24 lg:px-8">
          <motion.div initial={{ opacity: 0, y: 24 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6 }}>
            <p className="text-sm font-semibold uppercase tracking-[0.3em] text-gold">EMPOWER 40K</p>
            <h1 className="mt-4 text-4xl font-semibold leading-tight text-navy sm:text-5xl">Developing 40,000 entrepreneurs across West Africa by 2050.</h1>
            <p className="mt-6 max-w-3xl text-lg leading-8 text-navy/70">EMPOWER 40K is GGFG’s flagship regional initiative designed to build founders, strengthen communities, and accelerate a new generation of entrepreneurial leadership.</p>
          </motion.div>
        </div>
      </section>

      <section className="mx-auto max-w-6xl px-6 py-20 lg:px-8">
        <div className="rounded-[2rem] border border-navy/10 bg-[linear-gradient(135deg,_rgba(201,162,39,0.12),_rgba(255,255,255,1))] p-8 shadow-sm lg:p-12">
          <p className="text-sm font-semibold uppercase tracking-[0.3em] text-gold">A Future Worth Building</p>
          <h2 className="mt-4 max-w-3xl text-3xl font-semibold leading-tight text-navy sm:text-4xl">Imagine 40,000 young entrepreneurs across West Africa launching real businesses over the next generation.</h2>
          <p className="mt-6 max-w-4xl text-lg leading-8 text-navy/70">If, over time, each entrepreneur were able to create opportunities for just 10 other people, the ripple effect could exceed <span className="font-semibold text-navy">400,000 jobs</span> across the region.</p>
          <p className="mt-5 text-lg leading-8 text-navy/70">That is the scale of possibility behind <span className="font-semibold text-navy">EMPOWER 40K</span>.</p>
          <p className="mt-8 max-w-4xl text-base leading-8 text-navy/70">GGFG Nexus exists to make that future more achievable by developing entrepreneurs with the mindset, leadership, networks, mentorship, and ecosystem support they need to build sustainable ventures. While no outcome can be guaranteed, the potential is extraordinary. Every successful founder has the capacity to create opportunities far beyond their own business, strengthening families, communities, and local economies.</p>
          <p className="mt-8 max-w-3xl text-xl font-semibold leading-8 text-navy">Real transformation begins with one entrepreneur. Multiplied by 40,000, it has the potential to reshape the future of West Africa.</p>
        </div>
      </section>
      ...
      <section className="mx-auto max-w-6xl px-6 py-20 lg:px-8">
        <div className="grid gap-8 lg:grid-cols-[1fr_0.9fr]">
          <div className="rounded-[2rem] border border-navy/10 bg-[#F7F8FA] p-8">
            ...
            <h2 className="mt-3 text-3xl font-semibold text-navy">Too many promising founders are left without the systems, networks, and leadership support needed to scale.</h2>
            <p className="mt-5 text-base leading-8 text-navy/70">The next generation of businesses will not emerge from ideal conditions alone. They will emerge from disciplined development, strategic support, and long-term commitment.</p>
          </div>
          <div className="rounded-[2rem] border border-navy/10 bg-navy p-8 text-white">
            <p className="text-sm font-semibold uppercase tracking-[0.3em] text-gold">Why EMPOWER 40K</p>
            <h3 className="mt-3 text-3xl font-semibold">It is a movement that links talent, leadership, and enterprise into a scalable regional architecture.</h3>
            <p className="mt-5 text-base leading-8 text-white/70">We create the conditions for founders to move from idea to institution with clarity, resilience, and staying power.</p>
          </div>
        </div>
      </section>
        ...
        <div className="grid gap-8 lg:grid-cols-[0.95fr_1.05fr]">
          <div className="rounded-[2rem] border border-navy/10 bg-[#F7F8FA] p-8">
            ...
            <div className="mt-6 flex flex-wrap gap-3">
              {pillars.map((pillar) => (
                <span key={pillar} className="rounded-full border border-navy/10 bg-white px-4 py-2 text-sm font-medium text-navy/80">{pillar}</span>
              ))}
            </div>
          </div>
          <div className="rounded-[2rem] border border-navy/10 bg-white p-8 shadow-sm">
            ...
            <div className="mt-6 space-y-4">
              {[
                ...
                <div key={title} className="rounded-[1.25rem] border border-navy/10 bg-[#F7F8FA] p-5">
                  <h3 className="text-lg font-semibold text-navy">{title}</h3>
                  <p className="mt-2 text-sm leading-7 text-navy/70">{body}</p>
                </div>
              ))}
            </div>
          </div>
      ...
      <section className="mx-auto max-w-6xl px-6 pb-24 lg:px-8">
        <div className="flex flex-col gap-6 rounded-[2rem] border border-navy/10 bg-white p-8 shadow-sm lg:flex-row lg:items-center lg:justify-between">
          <div>
